...
export default class Server {
	...
	private genre: GenreController;
	private watchlist: WatchlistController;
	...
	constructor(serverOptions: ServerOptions) {
		...
		this.review = new ReviewController(this.sql);
		this.genre = new GenreController(this.sql);
		this.watchlist = new WatchlistController(this.sql)
		...
		this.review.registerRoutes(this.router)
		this.genre.registerRoutes(this.router);
		this.watchlist.registerRoutes(this.router)

		this.router.get("/", async (req: Request, res: Response) => {
			let user: User | null = null
			if (req.session.data.userId) {
				user = await User.read(this.sql, req.session.data.userId);
			}
